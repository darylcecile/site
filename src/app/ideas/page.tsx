	...
	return (
		<main className="relative mx-auto max-w-2xl px-6 pt-20 pb-12 sm:px-8 lg:px-0">
			<header className="mb-14">
				<p className="mb-4 font-mono text-xs uppercase tracking-widest text-muted-foreground">A working notebook</p>
			...
			{ideas.length ? (
				<div className="grid gap-6">{ideas.map((idea, index) => <IdeaCard key={idea.slug} idea={idea} index={index} />)}</div>
			) : (
