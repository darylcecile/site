...
export function Heading({ level = 1, ...props }: HeadingProps) {
	...
	return (
		...
		<HeadingTag {...props} className={cn(props.className, 'inline')} id={sectionId}>
			<Link
				href={`#${sectionId}`}
				className="relative before:right-full before:align-middle before:absolute before:content-['#'] before:text-foreground/50 before:mr-2 before:opacity-0 group hover:before:opacity-100 focus-visible:before:opacity-100 no-underline not-prose"
			>
				{props.children}
			</Link>
		</HeadingTag>
